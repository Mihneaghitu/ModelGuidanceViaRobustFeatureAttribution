...
def noisy_test_accuracy(
    ...
    *,
    ...
    noise_level = noise_level.to(device).type(param_n[-1].dtype)
    ...
    batch_n = transform(batch, model, 0)[0] if transform else batch.view(batch.size(0), -1, 1)
    ...
    *,
    ...
    for k, (param_l, param_n, param_u) in param_bounds_dict.items():
        ...
        certified_points = test_metrics.certified_predictions(
            param_n, param_l, param_u, batch, labels, model=model, transform=transform
        )
...
def compute_smooth_sensitivity(
    ...
    *,
    ...
    """
    k_max = compute_min_uncertified_k(batch, labels, param_bounds_dict, model=model, transform=transform)
...
def compute_local_epsilons(
    ...
    *,
    ...
    k_star = compute_min_uncertified_k(batch, labels, param_bounds_dict, model=model, transform=transform).cpu().numpy()
...
def get_calibrated_noise_level(
    ...
    *,
    ...
    if noise_type == "laplace":
        ...
        smooth_sens = compute_smooth_sensitivity(
            beta, batch, labels, param_bounds_dict, model=model, transform=transform
        )
